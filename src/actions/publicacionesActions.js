...
export const abrirCerrar = (pub_key, com_key) => (dispatch, getState) => {
  const { publicaciones } = getState().publicacionesReducer;
  const comentarioSeleccionado = publicaciones[pub_key][com_key];

  const comentarioActualizado = {
    ...comentarioSeleccionado,
    abierto: !comentarioSeleccionado.abierto
  };

  const publicaciones_actualizadas = [...publicaciones];
  publicaciones_actualizadas[pub_key] = [...publicaciones[pub_key]];
  publicaciones_actualizadas[pub_key][com_key] = comentarioActualizado;
...
export const traerComentarios = (pub_key, com_key) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({ type: COM_ERROR, payload: error.message });
  }
};
